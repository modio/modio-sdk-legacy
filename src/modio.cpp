  ...
  for(int i=0; i<(int)mod_events_array_size; i++)
  {
    switch( mod_events_array[i].event_type )
    {
      ...
      case EVENT_MODFILE_CHANGED:
      {
        ...
        modioInstallMod(NULL, mod_events_array[i].mod_id, (char*)modfile_path_str.c_str(), &onModfileChangedEvent);
      }
      break;
      ...
      case EVENT_MOD_EDITED:
      {
        ...
        modioGetMod(mod_path, mod_events_array[i].mod_id, &onModUpdateEvent);
        break;
      }
    }
  }
