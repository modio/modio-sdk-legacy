...
std::map< u32, GetModEventsParams* > get_mod_events_callbacks;
...
void modioOnGetModEvents(u32 call_number, u32 response_code, json response_json)
{
  ...
  get_mod_events_callbacks[call_number]->callback(get_mod_events_callbacks[call_number]->object, response, mod_events_array, mod_events_array_size);
  ...
  if(mod_events_array)
    delete[] mod_events_array;
  delete get_mod_events_callbacks[call_number];
  get_mod_events_callbacks.erase(call_number);
}
