...
void Instance::emailRequest(const std::string &email, const std::function<void(const modio::Response &)> &callback)
{
  ...
  email_request_calls[current_call_id] = (GenericCall *)email_request_call;

  modioEmailRequest((void *)new u32(current_call_id), email.c_str(), &onEmailRequest);
...
void Instance::emailExchange(const std::string &security_code, const std::function<void(const modio::Response &)> &callback)
{
  ...
  email_exchange_calls[current_call_id] = (GenericCall *)email_exchange_call;

  modioEmailExchange((void *)new u32(current_call_id), security_code.c_str(), &onEmailExchange);
