...
void Instance::getAllModDependencies(u32 mod_id, const std::function<void(const modio::Response &response, const std::vector<modio::Dependency> &mods)> &callback)
{
	...
	get_all_mod_dependencies_calls[current_call_id] = (GetAllModDependenciesCall *)get_all_mod_dependencies_call;

	modioGetAllModDependencies((void *)new u32(current_call_id), mod_id, &onGetAllModDependencies);
...
void Instance::addModDependencies(u32 mod_id, std::vector<u32> dependencies, const std::function<void(const modio::Response &response)> &callback)
{
	...
	add_mod_dependencies_calls[current_call_id] = (GenericCall *)add_mod_dependencies_call;

	modioAddModDependencies((void *)new u32(current_call_id), mod_id, dependencies_array, (u32)dependencies.size(), &onAddModDependencies);
...
void Instance::deleteModDependencies(u32 mod_id, std::vector<u32> dependencies, const std::function<void(const modio::Response &response)> &callback)
{
	...
	delete_mod_dependencies_calls[current_call_id] = (GenericCall *)delete_mod_dependencies_call;

	modioDeleteModDependencies((void *)new u32(current_call_id), mod_id, dependencies_array, (u32)dependencies.size(), &onDeleteModDependencies);
