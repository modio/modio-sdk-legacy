...
void Instance::oculusAuth(const std::string &nonce, u32 user_id, const std::string &access_token, const std::string &email, u32 date_expires, const std::function<void(const modio::Response &)> &callback)
{
  ...
  modioOculusAuth((void*)((uintptr_t)current_call_id), nonce.c_str(), user_id, access_token.c_str(), email_c, date_expires, &onOculusAuth);
