...
{
  this->modio_modfile_creator = new ModioModfileCreator;
  modioInitModfileCreator(this->modio_modfile_creator);
}
...
{
  modioSetModfileCreatorPath(this->modio_modfile_creator, (char *)path.c_str());
}

void ModfileCreator::setVersion(const std::string &version)
{
  modioSetModfileCreatorVersion(this->modio_modfile_creator, (char *)version.c_str());
}

void ModfileCreator::setChangelog(const std::string &changelog)
{
  modioSetModfileCreatorChangelog(this->modio_modfile_creator, (char *)changelog.c_str());
}

void ModfileCreator::setMetadataBlob(const std::string &metadata_blob)
{
  modioSetModfileCreatorMetadataBlob(this->modio_modfile_creator, (char *)metadata_blob.c_str());
}

void ModfileCreator::setActive(bool active)
{
  modioSetModfileCreatorActive(this->modio_modfile_creator, active);
}

ModioModfileCreator *ModfileCreator::getModioModfileCreator()
{
  return this->modio_modfile_creator;
}

ModfileCreator::~ModfileCreator()
{
  modioFreeModfileCreator(this->modio_modfile_creator);
}
