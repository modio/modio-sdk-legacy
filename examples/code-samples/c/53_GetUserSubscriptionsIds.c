...
void onGetAllSubz(void *object, ModioResponse response, ModioMod *mods, u32 mods_size)
{
  bool *wait = object;
  printf("Get dubz response: %i\n", response.code);
...
void onGetAllMods(void *object, ModioResponse response, ModioMod *mods, u32 mods_size)
{
  ...
  modioInitFilter(&filter);
    modioGetUserSubscriptions(&wait, filter, &onGetAllSubz);
